...
template<typename T>
__global__ void sparse_ce_loss_kernel(const T* pred,const float* targets,float* loss,int batch_seq,int vocab,int ignore_index)
{
    int row=blockIdx.x*blockDim.x+threadIdx.x;
    if(row<batch_seq)
    {
        int cls=(int)targets[row];
        if(cls!=ignore_index) atomicAdd(loss,-logf(to_float(pred[row*vocab+cls])+1e-7f)/(float)batch_seq);
    }
}
...
template<typename T>
__global__ void sparse_ce_backward_kernel(const T* pred,const float* targets,T* dY,int batch_seq,int vocab,int ignore_index)
{
    ...
    if(idx<batch_seq*vocab)
    {
        ...
        int cls=(int)targets[row];
        if(cls!=ignore_index) dY[idx]=from_float<T>((to_float(pred[idx])-(col==cls?1.0f:0.0f))/(float)batch_seq);
...
float sparse_ce_forward(const Tensor& pred,const Tensor& targets,int ignore_index)
{
    ...
    if(pred.dtype()==DType::F32) sparse_ce_loss_kernel<float><<<blocks,threads>>>(pred.get_data(),targets.get_data(),d_loss,batch_seq,vocab,ignore_index);
    else sparse_ce_loss_kernel<__nv_bfloat16><<<blocks,threads>>>((__nv_bfloat16*)pred.get_data(),targets.get_data(),d_loss,batch_seq,vocab,ignore_index);
...
void sparse_ce_backward(const Tensor& pred,const Tensor& targets,Tensor& dY,int ignore_index)
{
    ...
    int threads=256;
    int blocks=(total+threads-1)/threads;
    if(pred.dtype()==DType::F32) sparse_ce_backward_kernel<float><<<blocks,threads>>>(pred.get_data(),targets.get_data(),dY.get_data(),batch_seq,vocab,ignore_index);
    else sparse_ce_backward_kernel<__nv_bfloat16><<<blocks,threads>>>((__nv_bfloat16*)pred.get_data(),targets.get_data(),(__nv_bfloat16*)dY.get_data(),batch_seq,vocab,ignore_index);
}
