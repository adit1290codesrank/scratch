...
Tensor LayerNorm::forward(const Tensor& X,const Tensor* pad_mask)
{
    ...
    layernorm_forward(X.get_data(),g.get_data(),b.get_data(),Y.get_data(),X.total_elements()/dimension,dimension);
    return Y;
}
...
Tensor LayerNorm::backward(const Tensor& dY)
{
    ...
    layernorm_backward(dY.get_data(),cached_X.get_data(),g.get_data(),dg.get_data(),db.get_data(),dX.get_data(),dY.total_elements()/dimension,dimension);
    return dX;
}
